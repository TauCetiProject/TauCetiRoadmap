...
the sense of that roadmap; the discriminant group and forms computed there agree with those
computed in `E`. This is the *only* place the two models are compared; afterwards,
...
`∑_{γ ∈ A_L} e(q_L(γ)) = |A_L|^{1/2} e(n/8)`: the case `a = c = 1` of reciprocity. The
integral-lattices roadmap owns the general-signature statement; this is the instance Layer 7 and
`eight_dvd_finrank_of_even_unimodular'` need. -/
...
`E₈ ⊕ E₈` and `D₁₆⁺` (`README.md`, Layer 8E): equal theta series by `thetaForm_rank_16_eq`, and
