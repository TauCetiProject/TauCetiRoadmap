...
- the diamond action on `[Γ₁(N)]` is `P↦aP`. This is KM's own statement, immediately before
  Theorem 7.4.2: "The group `(ℤ/Nℤ)ˣ` operates on the moduli problem `[Γ₁(N)]` through its action
  on the given generator; an element `a` acts as `(E,P) ↦ (E,aP)`." It is also what Convention 5
  forces, since `(P,Q)·[[a,b],[0,d]] = (aP, bP+dQ)`. Inside `SL₂` it reads `P↦d⁻¹P`, because
  `ad=1` there; it is **not** `P↦dP`. So read, it agrees with the corresponding quotient of the
  full-level action, whose right action is KM (7.4.1), the same one Convention 5 fixes.

Prove the rigidity criterion for quotient problems and recover the semi-Borel rigid case for
`N≥4` (rigidity for `N ≥ 4` is KM 2.7.4, which is what KM itself cites at printed p. 120; 7.4.2(3) is
only the identification of `[Γ₁(N)]` with `[Γ(N)]` modulo the semi-Borel, and 7.4.2(4) the Borel
quotient giving `[Γ₀(N)]`; the rigidity criterion for
