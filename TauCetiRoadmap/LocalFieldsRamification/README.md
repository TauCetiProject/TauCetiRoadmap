...
- **Local monogenicity.** For every finite separable extension `L/K`, prove that the integer ring
  ...
  This is a local theorem: [the Number-Field Arithmetic roadmap](../NumberFieldArithmetic/README.md)
  consumes it only after passing to a completion. For an Eisenstein generator `ξ` of degree `n`,
  also prove the orthogonality of its
...
**Supplier status.** The dependency chain is **#188 → #244 → this roadmap**; #188 is merged and
#244 is open. `Suggested.lean` imports `TauCetiRoadmap.ProfiniteProPGroups.Suggested` directly, so
this branch's build is red until #244 lands and it is updated from `main` — which is the merge
order the review asks for, now enforced mechanically rather than by convention. Nothing is
duplicated or shadowed: there is no `Supplied.*` alias and no private replacement carrier for any
...
here) → this roadmap**. Within this roadmap the intended order is
