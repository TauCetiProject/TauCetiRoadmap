...
d² = 0, naturality under pullback). Almost none of the geometry usually built on such a core exists yet.
There are no differential forms on manifolds, no wedge product even on normed spaces, no manifold exterior
derivative, no orientability, no integration of forms, no Stokes' theorem, no de Rham or singular
cohomology, no flows (the integral-curve theory stops short of the maximal flow), no Frobenius theorem, no
Riemannian Laplacian, and no mapping degree.

This roadmap builds that theory, following Lee's *Introduction to Smooth Manifolds*. The main theorems are
the de Rham theorem (`H^p_dR(M) ≅ H^p(M; ℝ)`, by integration over smooth simplices), Stokes' theorem
...
theorem comes as an application, and the Laplace–Beltrami operator is the Riemannian endpoint. Everything
below these theorems is meant as reusable library. The geometric-topology, Heegaard Floer and Lie-groups
roadmaps all list parts of this material among their missing prerequisites, and the PDE roadmap's flat
conventions constrain the Laplacian built here.

The reference is John M. Lee, *Introduction to Smooth Manifolds*, 2nd edition, Springer GTM 218, cited as
[Lee] with that edition's numbering; where a layer takes a different route from the book, it says so.
Suggested homes, following Mathlib's layout (code organization and roadmap scope do not coincide; place
...
- **Scalars and regularity.** Everything is over `ℝ`, in Mathlib's setting: `I : ModelWithCorners ℝ E H`
  and `M` with `[ChartedSpace H M] [IsManifold I n M]`. Pointwise theorems are stated at the minimal
  finite regularity the flat layer demands (C² for the `extDeriv` facts, C³ for the Lie-bracket API). The
  global theory — the de Rham complex and everything cohomological — is fixed at C^∞, so that d maps
  smooth sections to smooth sections and no decreasing ladder of regularities appears. Analytic (`ω`)
  refinements are not targeted.
- **Finite dimension is a hypothesis, not a default.** Write `[FiniteDimensional ℝ E]` exactly where it
  is used: orientations, integration, Sard, degree, partitions of unity. Layers 0–1 and 3 stay
  Banach-general, matching Mathlib's `extDeriv` and `IsMIntegralCurve`. Smooth partitions of unity and
  smooth approximation additionally need `[T2Space M] [SigmaCompactSpace M]`, and every statement that
  uses them carries these hypotheses visibly. Second countability, where measurable structure needs it,
  follows from those two together with charts.
- **Boundary policy, per layer.** Layers 0–1 work over any `ModelWithCorners`, boundary included.
  Orientation (layer 2) is stated for a general `I`; note that ⚠ "orientable = admits a positive-Jacobian
  atlas" is *false* as a definition in the presence of boundary (see *Statements that must not enter*),
  and chart signs are exactly what make `Set.Icc x y` orientable. In layer 3, the local `IsMIntegralCurve`
  statements are consumed at interior points as Mathlib states them, while the maximal-flow theory of
  3.2–3.5 is stated for boundaryless `M`. Stokes' theorem (layer 5) is stated for manifolds with boundary;
  corners beyond `M × Icc` belong to 5.5.
- **The boundary model.** Mathlib's `EuclideanHalfSpace n` constrains the *first* coordinate; [Lee]'s `Hⁿ`
  constrains the *last*. All boundary-orientation signs (the `(−1)ⁿ` of [Lee, Example 15.26]) must
  therefore be recomputed for Mathlib's model and recorded as lemmas, not transcribed from the book. The
  only convention that transfers between the two models is "outward-pointing vector first".
- **Orientation.** Fibers reuse `Orientation ℝ (TangentSpace I x) ι`, with explicit
  ...
  wanted). A manifold orientation is chart-sign data, quotiented as in layer 2: the definition is by
  orientation transport, and the determinant criteria are derived from it.
- **Wedge normalization.** The determinant convention: `ω ∧ η = ((k+l)!/(k!·l!)) • Alt (ω ⊗ η)`. Under
  ...
  constants. Record the conversion to [Lee]'s Alt convention where the wedge is defined, and keep the two
  identities above as named lemmas, since they catch a mis-normalized wedge.
...
- **Total functions, junk values.** Operators follow the `mfderiv` convention: total, junk-valued outside
  the domain of good behaviour, with hypotheses on the theorems. Integration works the same way. Totality
  makes vacuous statements easy to write by accident, so every bundled object ships element-level
  `_apply`/`coe_` lemmas, and the acceptance gates pin concrete nonzero values.
- **Universes and vocabulary.** Spaces live in `Type` wherever the singular comparison is involved
  ...
  fields are unbundled, `V : (x : M) → TangentSpace I x`, with smoothness as a `ContMDiff` statement about
  the section; rough forms are likewise unbundled sections of the alternating bundle with a `ContMDiff`
  predicate; bundled types come last, and only at C^∞ (the design direction of the threads in the
  Acknowledgements). The d of a function *is* `mvfderiv`; never open-code `fderiv`-through-`extChartAt`.

## What Mathlib already has (consume)

Claims here are against the pin in this repository's `lake-manifest.json`. Re-grep before citing anything
in code, and treat every claim below that the pin lacks something as checkable. Individual layers cite the
further paths they consume.
...
- **Homological algebra, complete:** `Algebra/Homology/` (`CochainComplex (ModuleCat ℝ) ℕ`, `ShortComplex`
  ...
  (`CategoryTheory/Sites/MayerVietorisSquare.lean`, `Topology/Sheaves/MayerVietoris.lean`). All the
  homological algebra layers 6–9 need already exists; none of it is rebuilt here, and the only
...
- **Singular homology (not cohomology):** `AlgebraicTopology/SingularHomology/`
  ...
  and `alternatingCofaceMapComplex` (the dualization device of layer 8). The pin has no singular
  cohomology, no relative homology, no excision, no chain-level subdivision, no Mayer–Vietoris for the
  singular theories, and no cup product.
...
- **Smooth 2-forms:** `TauCeti/Geometry/Manifold/TwoForm.lean` (Heegaard Floer's symplectic lane;
  ...
  closedness is exported through layer 1, so that lane defines "closed" exactly once.
...
Everything the layers below specify; a grep of the pin finds none of it. The layer order is also the order
of dependence. Three items are substantially harder than their surroundings and gate what follows them:
the flat smooth-dependence theorem (3.1), the singular subdivision layer (8.2), and the smooth structure
on the boundary (5.1).
...
- **Mathlib's own direction.** The flat exterior calculus and its bundle substrate are Yury Kudryashov's
  ...
  accepted shape; for flows, mathlib4#26394 and #26395; for Levi-Civita, mathlib4#36845. In every case the
  repository policy applies: don't wait. Build the material here now, with the names and shapes those PRs
  use, and delete ours when the pin advances past them.
...
- **0.1 The wedge on `ContinuousAlternatingMap`,** in the determinant convention, at algebra-valued
  generality: define it through a continuous bilinear map on the values, so that the ℝ-valued and
  vector-valued wedges are a single definition. Prove bilinearity, associativity, graded commutativity,
  the norm bound with its explicit constant, compatibility with `compContinuousLinearMap`, the
  characterization through alternatization, and the two normalization identities of the conventions
  section. Also the **interior product** `ι_v`, with `ι_v ∘ ι_v = 0` and the degree-(−1) antiderivation
  rule [Lee, Lemma 14.13]. There is no linear equivalence between alternating and multilinear map spaces
  to transfer any of this along; the proofs are direct.
- **0.2 The smooth alternating bundle:** the `ContMDiffVectorBundle` instance for
  ...
  first, then specialize to `E₁ = TangentSpace I` and `E₂ = Bundle.Trivial M F`, so that the cotangent,
  2-form and top-degree bundles are all instances of one construction.
- **0.3 Rough and smooth forms, unbundled first.**
  ...
  `mpullback f ω x := (ω (f x)).compContinuousLinearMap (mfderiv I I' f x)`, total and junk-valued in the
  style of `VectorField.mpullback`, with `mpullback_id`, `mpullback_comp`, linearity, preservation of
  smoothness, and the wedge-homomorphism property. Identify 0-forms with functions and d of a 0-form with
  `mvfderiv`. Support API (`tsupport`, restriction to `Opens M`), and the pointwise wedge with its
  graded-algebra identities. State every theorem in a `Within` form first, with `univ` specializations,
  mirroring the flat file: the manifold-with-boundary cases later in the roadmap need the `Within`
  versions.
- **0.4 Bundled C^∞ forms:** `Ω^k⟮I, M; F⟯` as `ContMDiffSection`, with `AddCommGroup`, `Module ℝ`,
  `CoeFun`, `ext` and `_apply`; the equivalence with `TauCeti.SmoothTwoForm` at `k = 2`; the graded
  algebra `Ω^•`. *Acceptance:* over `𝓘(ℝ, E)`, bundled forms coincide with the flat carrier and
...
- **1.1 Definition and well-definedness.** `mextDerivWithin I ω s x` and `mextDeriv`: conjugate through
  ...
  through `mfderivWithin`. Chart-independence is `extDerivWithin_pullback` applied to transition maps over
  `Set.range I`; its `UniqueDiffOn` and closure hypotheses hold there for any model with corners. Package
  the `writtenInExtChartAt` rewriting as a reusable lemma, since the whole layer runs on it. Then
  locality, ℝ-linearity, `mextDeriv = extDeriv` over `𝓘(ℝ, E)`, and `mextDeriv` of a 0-form = `mvfderiv`.
...
- **1.3 d² = 0 and preservation of smoothness.** `mextDeriv_mextDeriv` for C² forms. Also a new flat
  lemma: `extDerivWithin` of a `ContDiffOn n` form is `ContDiffOn (n−1)` on the same set, stated so that
  it applies on `Set.range I`, together with its manifold transfer — so at C^∞, d maps `Ω^k` to
  `Ω^(k+1)`. This lemma is absent from the pin and the de Rham complex depends on it, so it is a target
  here, not an assumption.
- **1.4 Cartan calculus.** The Leibniz rule, with unit constants; the invariant formulas for `mextDeriv`
  on 1-forms and in all degrees through `VectorField.mlieBracket` [Lee, Prop. 14.29, 14.32]; the interior
  product with vector fields; the **Lie derivative of forms**, defined without flows by the bracket
  formula [Lee, Cor. 12.33], its flow characterization being 3.4's target; and **Cartan's magic formula**
  [Lee, Thm. 14.35]. *Acceptance:* on `ℝ²`, `d(x dy) = dx ∧ dy` and `d(dx) = 0`.
...
- **2.1 Orientability and orientations,** in the chart-sign design of mathlib4#35376, named and shaped
  identically so that this layer can be deleted once the pin includes it. `OrientationLift I M ι` is a
  model orientation `Orientation ℝ E ι` together with chart signs `M → M → ℤˣ`, supported and continuous
  on `(chartAt H x).source` and compatible under tangent-space transport along coordinate changes;
  `Manifold.Orientation I M ι` is the quotient by the diagonal `ℤˣ`-flip. Then the Prop-class
  `Orientable`, the data-class `OrientedManifold`, `orientationAt`, `InvolutiveNeg`, `eq_or_eq_neg` on
  preconnected `M`, the `LocallyConstant`-twisting equivalence, and the bridges to the determinant
  criteria. The definition is orientation transport; the determinant criteria are theorems.
- **2.2 Orientation-preserving maps and volume forms.** Orientation-preserving and -reversing local
  diffeomorphisms, via `orientationAt` and `Orientation.map`; the pullback orientation [Lee, Prop.
  15.15]; products. Using layer 0: a nonvanishing section of the top alternating bundle determines an
  ...
  `[T2Space M] [SigmaCompactSpace M]` visibly). The characterization by consistently oriented atlases is
  a theorem for boundaryless `M` with `1 ≤ dim` [Lee, Prop. 15.6], never the definition.
- **2.3 Smooth structures on covers.** A covering space of a C^n manifold carries a canonical
  ...
  of C^m maps are C^m. This is independent infrastructure — the Lie-groups roadmap's simply connected
  covers need it — and it is the covering–manifold bridge in the direction the pin lacks.
- **2.4 The orientation double cover** [Lee, Ch. 15, "Orientations and Covering Maps"].
  ...
  `isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul` — ⚠ not from the sigma topology, which
  fails (see *Statements that must not enter*). The smooth structure comes from 2.3, and the total space
  carries a canonical orientation and the deck involution. The characterizations, for connected `M`:
  orientable ↔ the cover is disconnected ↔ the cover is trivial (`≃ M × Bool`) ↔ the cover admits a
  section; a nonorientable connected `M` has a connected oriented double cover, unique up to
  orientation-preserving isomorphism over `M` [Lee, Thm. 15.41, 15.42]. Then the deck-group and
  index-2-subgroup characterization, and, still for connected `M`: if `π₁(M, x)` has no index-2 subgroup
  then `M` is orientable, so in particular simply connected manifolds are orientable [Lee, Thm. 15.43].
  The covering-space inputs are consumed from Tau Ceti, not rebuilt. *Acceptance:* `Set.Icc x y` and the
  spheres are orientable, with orientation covers `≃ M × Bool`; the Möbius band (the total space of the
  line bundle over `Circle`, shaped like mathlib4#40652) is connected and nonorientable, with connected
  orientation cover.
...
- **3.1 Smooth dependence on initial conditions, flat.** The local flow of a `ContDiff ℝ n` vector field
  ...
  as the parameter. State it flat, in `Analysis/ODE` idiom. This is the one genuinely new piece of hard
  analysis in the layer; make it a milestone of its own rather than burying it inside the manifold
  theorem.
- **3.2 The maximal flow.** For a C^n (`1 ≤ n`) field `v` on a boundaryless `[T2Space M]`: the domain
  ...
  flows** [Lee, Thm. 9.12]: each `flowOf v · x` is the unique maximal integral curve from `x`; the domain
  is open and the flow jointly C^n on it (by 3.1); and the group law
  `flowOf v t (flowOf v s x) = flowOf v (s+t) x` holds whenever `(s, x)` and `(t, flowOf v s x)` lie in
  the domain, which forces `(s+t, x)` into it. Consequently each `flowOf v t` is a diffeomorphism between
  open sets, with inverse `flowOf v (−t)`. The **escape lemma** [Lee, Lemma 9.19, strengthened to a
  one-sided form]: a maximal integral curve whose domain is bounded above eventually leaves every compact
  set, and symmetrically below.
- **3.3 Completeness and homogeneity.** Complete fields; the uniform-time criterion; compactly supported
  ...
  complete field's flow is a `Flow ℝ M`, together with the `ContMDiff` statement that `Flow` cannot
  record. **Homogeneity:** for any two points of a connected boundaryless `M` there is a compactly
  supported field whose time-1 flow carries one to the other, giving a diffeomorphism smoothly isotopic
  to the identity; 10.4 uses this to move regular values. Reconciliation: the flow of an invariant field
  on a Lie group, as in `TauCeti/Geometry/Lie/`, *is* `flowOf` of that field.
- **3.4 Naturality, straightening, commuting flows.** `F`-related fields have `F`-conjugate flows [Lee,
  Prop. 9.6, 9.13]. The **straightening theorem** [Lee, Thm. 9.22], which is layer 4's base case. The Lie
  ...
  Thm. 12.37] — stated for the two carriers this roadmap builds, vector fields and forms; the pin has no
  general tensor fields, and none are targeted here. *Acceptance:* the flow of a constant field on `ℝⁿ`
  is translation, and the flow of the rotation field on `ℝ²` is the rotation action.
...
- **4.1 Distributions.** Rank-`k` C^n distributions as `D : (x : M) → Submodule ℝ (TangentSpace I x)`
  ...
  Sections, `mem`-API, and the annihilator ideal of forms vanishing on `D` (which uses layer 0).
- **4.2 Involutivity and integral manifolds.** `IsInvolutive I D`, with its local-frame reformulation.
  ...
  *equal to* `D` — not merely contained in it — and they are never assumed embedded. The easy direction,
  integrable ⇒ involutive [Lee, Prop. 19.3], with the noninvolutive pair `∂x + y ∂z, ∂y` on `ℝ³` as a
  named nonexample; and the 1-form criterion: `D` is involutive iff `dη` annihilates `D` whenever the
...
- **4.4 Foliations and the global theorem.** Foliations by immersed leaves with flat charts, and the
  **global Frobenius theorem** [Lee, Thm. 19.21]: the maximal connected integral manifolds partition `M`
  into leaves, each an immersed submanifold carrying its own topology — a type with a finer charted-space
  structure, not a `Set`. The first consumer is the Lie-groups roadmap's subalgebra ↔ subgroup
  correspondence, which needs honest immersed leaves. *Acceptance:* the leaves of a constant distribution
  on `ℝⁿ` are the parallel affine slices; `ker (dz − y dx)` on `ℝ³` is not integrable.
...
- **5.1 The boundary as a smooth manifold.** The smooth structure on `I.boundary M` as an (n−1)-manifold
  ...
  `github.com/Deicyde/lean-boundary-smooth-manifold` as the migration source. With it: outward- and
...
- **5.2 Integration of top-degree forms.** Measurable structure on `M` comes from the standing
  `[T2Space M] [SigmaCompactSpace M]` and finite-dimension package (which governs the whole 5–9 track)
  together with charts. For oriented `M` and a compactly supported top-degree form `ω`, define `∫_M ω`
  chart-locally as the Lebesgue integral of the coefficient, weighted by the chart's sign in the 2.1
  `OrientationLift` data, and glue with a `SmoothPartitionOfUnity`. Well-definedness is the Jacobian
  change of variables at interior points, where 2.2's "orientation-preserving ⇒ positive determinant"
  removes the absolute value from `|det|`; the boundary is discharged by 5.1's measure-zero lemma. Chart
  signs rather than positively-oriented atlases: manifolds with boundary force this choice. Then
  independence of the partition and atlas, linearity, `∫_{−M} ω = −∫_M ω`, positivity on
  positively-oriented orientation forms, diffeomorphism invariance [Lee, Prop. 16.6], and the
  extension-by-zero/bump API (`SmoothPartitionOfUnity.smul` with `tsupport` control), factored out as a
  reusable family, since 5.3, 7 and 9 all consume it.
...
- **5.4 Stokes for `M × Icc` and homotopy invariance.** The product `M × Set.Icc (0:ℝ) 1` for
  ...
  invariance: pullbacks of a closed form along smoothly homotopic maps differ by an exact form, through
  the explicit operator `h ω = ∫_0^1 ι_t^* (ι_{∂_t} ω) dt` [Lee, Ch. 17, "Homotopy Invariance"]. General
  corners are deliberately avoided: `M × Icc` is all that layers 6–11 need.
- **5.5 Two scoped generalizations, last:** Stokes on manifolds with corners, and densities with
  integration of functions on nonorientable manifolds [Lee, Ch. 16, "Densities"]. Both are in scope here
  and nowhere else, and neither blocks layers 6–12. *Acceptance:* `∫_{Icc 0 1} df = f 1 − f 0`; the
  **angular form** `circleAngularForm` — the 0.3 pullback of `x dy − y dx` along `S¹ ↪ ℝ²`, written `dθ`
  below — is a named target, with `∫_{S¹} dθ = 2π`; and Green's theorem on the unit square, from the flat
  box divergence theorem (the square has corners, so this gate belongs here and to the flat layer, not to
  5.3).
...
- **6.1 The complex.** `mextDerivₗ : Ω^k⟮I, M⟯ →ₗ[ℝ] Ω^(k+1)⟮I, M⟯` and
  ...
  Both presentations are targets: the concrete `deRhamCohomology I M k` as a `Submodule.Quotient`, with
  `IsClosed`/`IsExact` predicates and element-level API — this is what analytic arguments compute with,
  and what Heegaard Floer's "closed 2-form" consumes — and the categorical
  `(deRhamComplex I M).homology k`, with the comparison isomorphism between the two. Functoriality:
  `mpullback` as a cochain map (by 1.2), the induced map on cohomology, `pullback_id`, `pullback_comp` —
  as unbundled lemmas, with no manifold category.
- **6.2 Degree zero and the Poincaré lemma.** `H⁰ ≅` locally constant functions; `H^k(pt)`. The
  **Poincaré lemma in all degrees**: on a star-shaped open set of a normed space, a closed C¹ k-form
  (`k ≥ 1`) with values in a complete space is exact. State it flat, in the `extDerivWithin` idiom,
  beside Mathlib's 1-form convex case, with the explicit radial homotopy operator (a Bochner integral,
  hence the completeness hypothesis). The manifold corollary: closed forms are locally exact [Lee, Thm.
  17.14, Cor. 17.15].
- **6.3 Homotopy invariance.** Smoothly homotopic maps induce equal maps on `H^•` (via 5.4's operator);
  ...
  nothing in this layer waits on layer 8: the integral of a 1-form along a piecewise-C¹ path in `M`
  (pullback to the interval plus `intervalIntegral`, reconciled with `curveIntegral`), with homotopy
...
- **7.1 The short exact sequence.** For `U V : Opens M` with `U ⊔ V = ⊤`, the sequence
  ...
  `[T2Space M] [SigmaCompactSpace M] [FiniteDimensional ℝ E]` package — surjectivity is a
  partition-of-unity statement and fails without it. Restrictions come from 0.3; the sign convention is
  `(res, −res)`, matching Mathlib's sheaf-level Mayer–Vietoris, so the two sequences are comparable
  arrow-for-arrow; surjectivity is by the extension trick of 5.2.
...
- **7.3 The compactly supported variant**
  ...
  form (from 2.2, detected by 5.2's positivity and 5.3's `∫ dη = 0`). This computation is the input to
  layer 10.
...
smooth simplices and a Mayer–Vietoris induction (de Rham–Weil). The sheaf route through `Sheaf.H` was
considered and is not taken: the pin has neither fine-sheaf acyclicity nor the Čech/derived comparison, so
it is a longer detour than the chain-level argument.*
...
- **8.2 Subdivision and Mayer–Vietoris, singular.** Barycentric subdivision as a chain map
  ...
  given by the standard smooth loop simplex (8.5's acceptance gate evaluates against it). This item is
  the largest single piece of work in the second half of the roadmap: the pin has no chain-level
  subdivision at all — its only subdivision is the simplicial-set functor `SSet.sd`, which comes with no
  comparison to the identity. Excision is not a target; Mayer–Vietoris is all the comparison needs.
- **8.3 Smooth chains.** Smooth singular simplices and chains, and the smoothing operator: the inclusion
  ...
  serves every manifold, not only compact ones — and the ε-neighbourhood retraction of the embedded
  image. The intrinsic tubular-neighbourhood theory stays with the geometric-topology roadmap.
...
- **8.5 The de Rham theorem.** `I` is an isomorphism in every degree, for every T2 σ-compact
  ...
  *Statements that must not enter*). The de Rham–Weil induction runs in two stages. First, every open
  ...
  this is why the stage is separate. Second, the manifold case: charts, a named
  diffeomorphism-invariance lemma for the de Rham property, Mayer–Vietoris on both sides, countable
  disjoint unions, and the Poincaré lemma as base case. Do not route through geodesic convexity; the
  ...
  whose value on the fundamental class of `S¹` (8.2's target) is `2π`, reusing 5.5's number to catch
  orientation drift.
...
- **9.2 Top cohomology.** Integration `H^n_c(M) → ℝ` is an isomorphism for connected oriented `M` [Lee,
  ...
  17.34, via Lemma 17.33]; this is where the orientation double cover does real work. (For connected
  noncompact `M`, `H^n_dR(M) = 0` is a corollary of 9.3: `H^n ≅ (H^0_c)* = 0`.) In the compact connected
  oriented case, `H^n_dR(M) ≅ ℝ` via `∫_M`.
- **9.3 The duality theorem.** `PD` is an isomorphism for every T2 σ-compact oriented boundaryless
  ...
  proof is a Mayer–Vietoris induction with the five lemma, base case 6.2/6.4, following the template of
  8.5. Corollaries: for compact oriented `M`, `H^k ≅ (H^{n−k})*`; finite-dimensionality of all the
  `H^k_dR` of a compact manifold; and the vocabulary of the top-cohomology pairing. The Hodge-theoretic
  proof is deliberately not used: elliptic theory belongs to the PDE roadmap, and duality must not depend
  on it. *Acceptance:* duality on `Sⁿ` and `ℝⁿ` recovers 7.3 and 6.4; `⟨[dθ], [1]⟩ = ∫_{S¹} dθ = 2π ≠ 0`,
  pairing against the class of `1` in `H⁰_c(S¹)`.
...
- **10.1 The manifold inverse function theorem.** If `mfderiv` is invertible at `x`, then
  ...
  maps, in the chart-normal-form idiom. This is the first target of the layer, and is useful well beyond
  it.
...
- **10.3 The stack of records and the orientation-theoretic degree.** For a C^∞ map `f : M → N` with `M`
  ...
  `deg f := Σ_{x ∈ f⁻¹ y} sign (mfderiv f x)`, the sign taken through 2.1's `orientationAt`.
...
- **10.5 Degree calculus.** `deg (g ∘ f) = deg g · deg f` (with `N` compact and `P` connected, so that
  ...
  target: `SmoothApprox` into the ambient space, radial normalization, and the straight-line homotopy,
  with well-definedness on homotopy classes. This is enough for every corollary here; the extension to
  general targets goes through 8.3's Whitney approximation and is a separate, later item.
  `deg (antipodal : Sⁿ → Sⁿ) = (−1)^{n+1}`. Corollaries: there is no retraction `Sⁿ ↛ B^{n+1}`, and the
  Brouwer fixed-point theorem in all dimensions. Reconciliation rather than duplication: on `S¹`, `deg`
...
- **12.1 Existence and examples of metrics.** Every second-countable T2 finite-dimensional C^∞ manifold
  ...
  cone [Lee, Prop. 13.3]. Pullback metrics along immersions, product metrics, the induced metric on
  `Opens` and on 5.1's boundary manifold, and the round metric on spheres — the pullback of the ambient
  inner-product metric along the inclusion, which 12.5's volume gate uses. Built here now, shaped like
  mathlib4#33714, and deleted when the pin includes it.
...
- **12.3 The Levi-Civita connection.** Metric-compatibility and torsion for a `CovariantDerivative` on
  ...
  computation, and the value at critical points. `laplaceBeltrami f := div (mgradient f)`, equal to the
  trace of the Hessian, registered in the `Δ` notation class.
...
- **12.5 The divergence theorem and Green's identities.**
  ...
  `Δ‖x‖² = 2n` on `ℝⁿ`; and the volume of `S¹` with the round metric is `2π`, reusing 5.5's integral.

## Statements that must not enter the library

Each of these is a tempting formalization error, given with a concrete refutation. Reject any target or
proof that smuggles one in.
...
- **Geometric topology** consumes the orientation interface (its connected-sum and surgery layers
  ...
  seams are shared, resolved as follows. The boundary-as-manifold structure, which its gluing track names
  as its first prerequisite, is 5.1's target — one file serving both roadmaps. The general distribution
  and foliation objects are layer 4's, with its codimension-one layer specializing them, so a single
  foliation definition enters the library. The Riemannian volume form, measure and densities are 12.4's
  and 5.5's, consumed by its hyperbolic-volume targets. Intrinsic tubular and collar neighbourhoods,
  gluing, and everything 3-manifold-specific stay with it.
- **PDE** owns everything analytic about `Δ` beyond 12.5 and pins the flat conventions 12.5's gate must
  match: the operator is defined here and analyzed there.
