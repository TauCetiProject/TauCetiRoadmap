...
three facts this roadmap's own Sylow, free-object and rank milestones consume. This roadmap also
owns the elementwise power `proPPow x a` for `a : ℤ_p` in an arbitrary profinite pro-`p`
group, obtained in
the abelian procyclic closure of `x`; the dyadic normal-form words require precisely this special
case, not a `ℤ̂`-action on the whole nonabelian group. `BelyiMaps`
...
- **Elementwise exponentiation in any profinite pro-`p` group.** For `x : G`, apply the preceding
  construction to the abelian procyclic closure of `x` and compose with its inclusion in `G`.
  This gives `proPPow x a` for `a : ℤ_p`, with the natural-power, additive-exponent and continuity
  laws, naturality under continuous homomorphisms, and membership in the procyclic closure of
  `x`. It is the operation used by the `α ∈ 4ℤ₂` and `f = ∞` Demushkin words. It is not a global
  scalar action on a nonabelian group.
  *Needs:* L3 closed-subgroup `IsProP`; the abelian exponentiation immediately above.
...
- **Power-series coordinates.** For a chosen topological generator `γ` of `Γ ≅ ℤ_p`, the
  ...
  `PowerSeries` carries no topology at the pin, so the topological half is stated algebraically.
  For `U` open, the finite-level kernel corresponds to the principal ideal generated by
  `ω_U = (1+T)^[Γ:U] - 1`, not to a power of `T`. The full inverse-limit topology also sees the
  `p`-adic topology on each finite group algebra: the neighbourhood over `(U,s)` corresponds to
  the two-generated ideal `(p^s, ω_U)`. These are
  `powerSeriesCoordinate_finiteLevelKernel` and `powerSeriesCoordinate_neighborhood`.
...
- **The marked classification.** For each normal-form family, the classification is stated in
  ...
  `demushkinWordNeTwo`, `demushkinWordTwoOdd` and `demushkinWordTwoEven`. The dyadic words live in
  the free pro-`2` group and use `proPPow`: their parameters are literally `f : ℕ∞` and
  `α : ℤ₂`, so `f = ∞`, `α = 0`, and nonintegral `2`-adic values are expressible. The character
  ...
  parameters are **outputs** of the two `q = 2` classification theorems, not free inputs checked
  by membership of one unit in the orientation image. The unmarked isomorphism statements are
  corollaries, obtained by forgetting the character clause. The marked form supplies the
  normalization consumed by `LocalGaloisGroups`.
