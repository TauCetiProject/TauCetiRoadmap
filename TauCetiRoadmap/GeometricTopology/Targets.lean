...
groups; locally flat embeddings; diffeomorphism-group topologies; Dehn surgery;
Riemannian volume and hyperbolic structures; geometric decomposition; Heegaard genus;
foliations; triangulations and collapse), each attached to the solved Kirby-list problems
it would let us *state*. Knot theory and concordance (layers 4 and 6) are consumed from
the [combinatorial Heegaard Floer roadmap](../CombinatorialHeegaardFloer/Targets.lean)
rather than rebuilt.
...
topology, no Dehn surgery, no Riemannian volume or hyperbolic structures, no Heegaard
genus, no foliation Euler class, no triangulation-of-a-manifold. So, exactly as in the
Heegaard Floer and Universal Covers roadmaps, there are no compiled targets to state
against the pin yet: the whole point of the issue is that these theorems are not statable
today.
