...
- **Triangulation of a space** in the general sense: `IsTriangulable M := ∃ K, Nonempty (|K| ≃ₜ M)`.
  ...
  the dimension-3 existence, [extract](references/moise-triangulation-dim3.md).)  The dimension 2 case, Rado's theorem, is handled by the [Planar Topology](../PlanarTopology/README.md) roadmap.
- **Combinatorial manifolds via the link condition** (Kim's point, the simplicial side of
  ...
  **combinatorial `n`-manifold** if the link of every vertex is a combinatorial
  ...
  that "has the correct links to be a manifold". The generalized maps of the [Surface Topology](../SurfaceTopology/README.md) roadmap give a complementary, dart-based encoding of local manifold structure.  In dimension 2 the condition is decidable and recursion is unnecessary.
