...
/-- Flows whose arrow values lie in the cyclic subgroup generated by `d`: integer flows for
`d = 1`, rational flows with common denominator `D` for `d = 1 / D`. -/
def CyclicFlow (s t : V) (d : K) : Type _ :=
  {f : Flow Q cap s t // ∀ {v w} (e : Q v w), f.toFun e ∈ AddSubgroup.zmultiples d}

/-- For capacities in `zmultiples d` with `0 < d`, the values of flows in the subgroup are
multiples of `d` bounded by a multiple of `d`, so every sequence of strictly value-increasing
such flows, in particular every sequence of augmentations starting from one, is finite.
No Archimedean assumption is needed. -/
theorem CyclicFlow.wellFounded_val_lt {d : K} (hd : 0 < d)
    (hcap : ∀ {v w} (e : Q v w), cap e ∈ AddSubgroup.zmultiples d) :
    WellFounded (fun g f : CyclicFlow Q cap s t d => f.1.val < g.1.val) := by
  sorry

/-- Over `ℤ` every flow has arrow values in the subgroup generated by `1`. -/
theorem Flow.wellFounded_val_lt_int (cap : Assignment Q ℤ) :
    WellFounded (fun g f : Flow Q cap s t => f.val < g.val) := by
  sorry

/-- Finitely many rational capacities have a common denominator. -/
theorem exists_common_denominator (cap : Assignment Q ℚ) :
    ∃ D : ℕ, 0 < D ∧ ∀ {v w} (e : Q v w), cap e ∈ AddSubgroup.zmultiples ((D : ℚ)⁻¹) := by
  sorry

theorem CyclicFlow.wellFounded_val_lt_rat (cap : Assignment Q ℚ) {D : ℕ} (hD : 0 < D)
    (hcap : ∀ {v w} (e : Q v w), cap e ∈ AddSubgroup.zmultiples ((D : ℚ)⁻¹)) :
    WellFounded (fun g f : CyclicFlow Q cap s t ((D : ℚ)⁻¹) => f.1.val < g.1.val) := by
  sorry
...
open Classical in
/-- Numerical vertex connectivity, kept beside the [#42494](https://github.com/leanprover-community/mathlib4/pull/42494) stand-ins until Mathlib settles its API.
...
theorem isVertexConnected_iff_le_vertexConnectivity [Finite V] [Nonempty V] (k : ℕ) :
    IsVertexConnected G k ↔ (k : ℕ∞) ≤ vertexConnectivity G := by
  sorry
...
theorem vertexConnectivity_ne_top [Finite V] : vertexConnectivity G ≠ ⊤ := by
  sorry
...
theorem vertexConnectivity_le_edgeConnectivity [Finite V] :
    vertexConnectivity G ≤ edgeConnectivity G := by
  sorry
