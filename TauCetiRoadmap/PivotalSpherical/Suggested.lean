...
⚠ `[IsAlgClosed k]` is equally load-bearing, and is why it is carried here rather than only at the
`FPdim` bar. Over `ℝ`, finite-dimensional representations of `C₃` satisfy every other hypothesis,
but the irreducible two-dimensional rotation representation `V` has `End V ≅ ℂ` as an `ℝ`-algebra,
so `fusionCoeff ℝ 𝟙 V V = dim_ℝ Hom(𝟙 ⊗ V, V) = 2` while `V` occurs in `𝟙 ⊗ V` with multiplicity
one. An explicit split hypothesis `End S ≃ₐ[k] k` on every simple `S` would do just as well; the raw
`k`-dimension is never the multiplicity over a nonsplit endomorphism algebra. See EGNO §4.16. -/
noncomputable def fusionCoeff (k : Type w) [Field k] [IsAlgClosed k] {C : Type u} [Category.{v} C]
...
def grothendieckRing (k : Type w) [Field k] [IsAlgClosed k] (C : Type u) [Category.{v} C]
...
/-- **The adjoint subcategory** `C_ad`: the subcategory generated by the objects `X ⊗ Xᘁ` for
**simple** `X` (EGNO Definition 4.14.5), presented here as the generating set indexed by
`SimpleClasses C` so that the quantifier cannot be lost.

⚠ Allowing an arbitrary `X` is not a harmless generalization, it changes the subcategory. In
`Vec_{C₂}` with `g` the nontrivial simple object, `X = 𝟙 ⊕ g` has `X ⊗ Xᘁ ≅ 𝟙 ⊕ 𝟙 ⊕ g ⊕ g`, whose
closure under direct summands is the whole category, whereas the actual adjoint subcategory of a
pointed category is its neutral component. Acceptance: `C_ad (Vec_{C₂}) = Vec` together with
`U (Vec_{C₂}) = C₂`. -/
def adjointGenerators {C : Type u} [Category.{v} C] [MonoidalCategory C] [RigidCategory C]
    [Limits.HasZeroMorphisms C] : Set C :=
  {Y | ∃ i : SimpleClasses C, Y = i.repr ⊗ (i.repr : C)ᘁ}

/-- **The universal grading group** `U(C)` of a fusion category: the group carrying the finest
faithful grading, with trivial component the adjoint subcategory generated by
`adjointGenerators` (Gelaki–Nikshych; DGNO10). Consumes the Grothendieck based ring of Layer 4. -/
...
⚠ This is exposed as a named `def` as well, because the ribbon/spherical comparison below is about
*this* twist: the twists on a braided rigid category form a torsor over the monoidal natural
automorphisms `u` of the identity, and `θ · u` is ribbon only when `u_{Xᘁ} = (u_X)ᘁ`. -/
noncomputable def balancedOfBraidedPivotal (C : Type u) [Category.{v} C] [MonoidalCategory C]
    [BraidedCategory C] [RigidCategory C] [PivotalCategory C] : BalancedCategory C := sorry

@[inherit_doc balancedOfBraidedPivotal]
theorem nonempty_balanced_of_braided_pivotal (C : Type u) [Category.{v} C] [MonoidalCategory C]
    [BraidedCategory C] [RigidCategory C] [PivotalCategory C] : Nonempty (BalancedCategory C) :=
  ⟨balancedOfBraidedPivotal C⟩

/-- **Ribbon implies spherical**, unconditionally: this is the direction of HPT Appendix A.2,
Proposition A.4 that needs no semisimplicity, and it is what pins the correspondence. Stated against
the named twist of `balancedOfBraidedPivotal`, since a hypothesis about an arbitrary balanced
structure would be about a different twist. -/
theorem spherical_of_ribbon (C : Type u) [Category.{v} C] [MonoidalCategory C] [BraidedCategory C]
    [RigidCategory C] [PivotalCategory C]
    (_h : letI := balancedOfBraidedPivotal C; RibbonCategory C) : SphericalCategory C := sorry
...
one point of it; a theorem taking some centre pivotal structure as input would be about a different
...
theorem centerPivotalOfPivotal_forgets (C : Type u) [Category.{v} C] [MonoidalCategory C]
    ...
    (pivotalIsoApp X).hom.f ≫ (centerRightDualIso C ((X : Center C)ᘁ : Center C)).hom ≫
      ...
      = (pivotalIsoApp X.1).hom := sorry

/-- **The twist induced on the centre by a spherical structure.** `centerPivotalOfPivotal` makes
`Z C` braided pivotal, and eq (3) turns that into a twist; this pins *that* twist, which is the one
...
⚠ Two hypotheses here are load-bearing.

Quantifying over an arbitrary `[BalancedCategory (Center C)]` would be **false**. Given a ribbon
twist `θ` and a monoidal natural automorphism `u` of the identity, `θ · u` is again a twist, and it is
ribbon only when `u_{Xᘁ} = (u_X)ᘁ`. The theorem has to name the twist it means, which is why it is
stated against `centerBalancedOfSpherical`.

The fusion hypotheses are equally necessary, and this is the one centre arrow the roadmap does not
state at bare generality. HPT Appendix A.2, Proposition A.4 proves ribbon ⟹ spherical without them
(`spherical_of_ribbon`) and adds semisimplicity for the converse. Dropping it is refuted in
characteristic three. In `FDRep k C₃` with its symmetric braiding, twist the standard pivotal
structure by the central generator `g`. Writing `ρ(g) = I + N` we have `N³ = 0`, and every equivariant
`f` commutes with `N`, so `N f` is nilpotent and `Tr(ρ(g) f) = Tr(f) = Tr(ρ(g⁻¹) f)`; those are the
two pivotal traces, so this structure is spherical in the sense of `SphericalCategory`. Its twist is
`ρ(g)`, which is not ribbon: on the two-dimensional representation with `ρ(g) = [[1,1],[0,1]]` the
action on the dual is `ρ(g⁻¹)ᵀ` while the dual of the twist is `ρ(g)ᵀ`, and these differ in
characteristic three. The symmetric embedding `C → Z(C)` preserves both the induced pivotal structure
and the braiding, so the induced twist on `Z(C)` restricts to that non-ribbon twist. -/
theorem ribbon_center_of_spherical (k : Type w) [Field k] [IsAlgClosed k] [CharZero k] (C : Type u)
    [Category.{v} C] [MonoidalCategory C] [RigidCategory C] [Abelian C] [Linear k C]
    [MonoidalPreadditive C] [MonoidalLinear k C] [Simple (𝟙_ C)]
    [∀ X Y : C, FiniteDimensional k (X ⟶ Y)] [IsFiniteSemisimpleCategory C]
