...
- Is the regularity-to-counting bridge **named** — the concrete lower-route counts, the
  ...
  **complexity-bounded** calibration against `inducedCountingRank3` — never a bare `1 ≤ r`, and
  never a calibration over arbitrary complexes, which no fixed rank could satisfy)? Does
  `placed_induced_counting3` take only its local contract (transversality, lower-skeleton
  regularity at the schedule, route-local top regularity, rank adequacy) — never the full
  approximation bundle? Is the discarded **predicted** mass transversal-only, with the
  repeated-cell predicted side owned by the diagonal gate (predicted-side partition
  overlap-free, no hidden repeated-cell
  counting lemma)?
