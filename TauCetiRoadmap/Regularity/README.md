...
- Are top relations a **total unordered** coloring with a **separate** pair palette `κ₂` / top palette
  ...
  placements and counting **in the approximant `H'`** (never per-placement in `H`), the named
  global edit transfer (`inducedCopyCount_edit_transfer`), the diagonal floor
  (`VertexCellsControlled` at `diagonalControl3 F₀.k ε`), the pattern-size error scale `|V|^{F₀.k}`
  (not `|V|³`), and **`V`-independent** regularity parameter and rank `inducedCountingParameter3` /
