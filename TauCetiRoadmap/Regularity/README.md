...
- Are top relations a **total unordered** coloring with a **separate** pair palette `κ₂` / top palette
  ...
  only most polyads, so unrestricted routes would claim control over exceptional polyads) and
  counting **in the approximant `H'`** (never per-placement in `H`), the named step-2 union bound
  (`exceptional_route_mass_le`), the named global edit transfer
