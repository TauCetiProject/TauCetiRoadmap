...
- Are hypergraph complexes, cells, polyads, subpolyads, and relative densities **real targets**, not
  ...
  `IsVertexBoxRegularOverPolyad` never consumed by the counting summit? Are polyad supports
...
- Does `TriadicComplex3` **choose** its lower pair palette (`pairColorCount`), and does the summit
  ...
  `t₀`** (a `t₀`-independent bound makes the summit false for `t₀` above it)? Does the summit's
...
- Are top relations a **total unordered** coloring with a **separate** pair palette `κ₂` / top palette
  ...
  **in the approximant `H'`** (never per-placement in `H`), and at the **per-route budget**
  `routeBudget3` (a bare per-route `ε` cannot sum over the `q₂^(k choose 2)` routes per placement
  to the global bound)? Are the discarded routes bounded on **both** sides — actual
  (`exceptional_route_mass_le`) and predicted (`exceptional_route_prediction_mass_le`, consuming
  lower-skeleton regularity at the schedule)? Does the chain use the named global edit transfer
  (`inducedCopyCount_edit_transfer`), the diagonal floor (`VertexCellsControlled` at
  `diagonalControl3 F₀.k ε`), the pattern-size error scale `|V|^{F₀.k}` (not `|V|³`), a genuine
  **schedule** `inducedCountingSchedule3` separate from the global parameter
  `inducedCountingParameter3` (never the counting error `ε` itself as the regularity strength, and
  never a constant schedule), and a rank pinned `≥ 1` (`one_le_inducedCountingRank3` — rank `0` is
  vacuous)?
