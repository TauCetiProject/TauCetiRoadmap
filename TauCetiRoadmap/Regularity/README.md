...
- Is the regularity-to-counting bridge **named** — the concrete lower-route counts, the
  ...
  `pairRouteRegularityThreshold3` with **both** its positivity and its calibration against the
  schedule (decoupling input strength from output slack without a vacuous instantiation), and the
  dense conversion's explicit parameter inequalities (`δ ≤ ρ`, `η ≤ δ`)? Does
