...
- Are top relations a **total unordered** coloring with a **separate** pair palette `κ₂` / top palette
  ...
  **in the approximant `H'`** (never per-placement in `H`), and at the **per-route budget at the
  ...
  (`exceptional_route_prediction_mass_le`, with **separate** `εmass`/`εcount`, the schedule charged
  at the **lower complexity** it is actually supplied at, and the **route-count factor** in the
  slack)? Do the **charge-pinning inequalities** (`inducedCountingParameter3_charge`,
  `inducedCountingSchedule3_charge`) show every global charge fits its `ε/6` fraction? Does the
  chain use the named global edit transfer (`inducedCopyCount_edit_transfer`), the diagonal floor
  (`VertexCellsControlled` at `diagonalControl3 F₀.k ε`), the pattern-size error scale `|V|^{F₀.k}`
  ...
  `inducedCountingSchedule3_pos`, needed for the summit's `hF`), and a rank pinned `≥ 1`
