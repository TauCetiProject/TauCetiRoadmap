...
analytic locus `Spa(A,A⁺)^a`.

Proposition 7.49(2) rests on the following inputs, which are milestones here rather than steps
inside its proof. An analytic valuation is **microbial** (Remark 7.40(5)): some convex subgroup of
the value group it attains has height-one quotient. State this without introducing a theory of
height — "height at most one" is the condition that the only convex subgroups are `⊥` and `⊤`, and
height exactly one adds nontriviality — and state it on the value group the valuation actually
attains rather than on the ambient group of units, since the ambient reading is satisfied by
trivial valuations, whose attained value group is not of height one. Prove that a microbial
valuation admits a **vertical generization whose value group has height one**, by coarsening along
the convex subgroup the definition supplies, and pin the predicate from both sides: every rank-one
valuation is microbial, and a valuation with trivial value group is not. Prove **Lemma 7.45**: for
a complete affinoid ring and a prime ideal that is not open, there is an analytic point of height
one whose support contains that prime; include the refinement under a noetherian hypothesis, where
the ideal of definition is principal and the valuation may be taken discrete. The height-one
vertical generization is what the proof of Proposition 7.49(2) runs on, and Lemma 7.46(2) (§5.2)
consumes Lemma 7.45 in turn.

Prove Proposition 7.49(2), including the criterion for this locus to
...
References: Wedhorn §§8.2–8.4; Huber [Hu2].
...
Define an **adic** morphism of Huber pairs (Definition 8.38): a continuous ring map `φ : A → B` is
adic when the ideal generated by the image of an ideal of definition of a ring of definition of `A`
is an ideal of definition of a ring of definition of `B`. Neither Mathlib nor this development has
a predicate for an adic homomorphism — Mathlib's `IsAdic`, `IsAdicComplete` and `IsAdicCauchy` are
conditions on a topology, not on a map — so the predicate is itself a target. State it so that it
does not depend on the chosen pairs of definition, and give it the API its consumers need:
identity and composition are adic, and adicness is equivalent to the openness of the extended ideal
of definition, which is the form the analytic-locus argument below uses. Ideals of definition are
comparable only up to powers, so a containment between a chosen ideal of definition of `B` and the
extension of one of `A` is strictly stronger than adicness and is not the right condition.

Prove **Lemma 7.46**, whose parts carry different hypotheses and are stated separately. Part (1),
for a continuous `φ : A → B` and `f = Spa(φ)`: `f` carries the non-analytic locus into the
non-analytic locus, which needs only continuity, since the support of a pullback is the preimage of
the support and an open support stays open; and if `φ` is adic, `f` carries the analytic locus into
the analytic locus. State both directions and the resulting locus equality
`f⁻¹(Spa(A,A⁺)^a) = Spa(B,B⁺)^a`. Part (2) consumes Lemma 7.45 (§2.3) and needs the completeness
hypothesis carried there; state it at that strength and not under continuity alone.

Prove **Proposition 8.39**: a morphism is adic if and only if it maps the analytic locus into the
analytic locus, while every morphism maps the non-analytic locus into the non-analytic locus. Its
proof is Lemma 7.46 directly, so nothing further is needed once that lemma is available at the
right hypotheses. Prove **Corollary 8.40**, the induced maps of affinoid rings attached to an adic
morphism of affinoid adic spaces.

### 5.3 Morphisms locally of finite type
